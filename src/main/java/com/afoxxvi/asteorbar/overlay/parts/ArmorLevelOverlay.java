...
public class ArmorLevelOverlay extends BaseOverlay {
    private void draw(PoseStack poseStack, int left, int top, int right, int bottom, int armor, double armorToughness, boolean flip) {
        ...
        GuiHelper.drawTexturedRect(poseStack, left, top, right, bottom, 9, Y_ARMOR_EMPTY_BAR, BOUND_FULL_WIDTH_SHORT, 9);
        drawTextureFillFlip(poseStack, left + 1, top, right - 1, armorWidth, 9, 10, Y_ARMOR_BAR, flip);
        if (armorToughness > 0) {
            int armorToughnessWidth = (int) ((right - left) * Math.min(1.0, armorToughness / 20.0));
            drawTextureBoundFlip(poseStack, left, top, right, bottom, armorToughnessWidth, 9, 9, Y_ARMOR_BOUND, BOUND_FULL_WIDTH_SHORT, flip);
        }
    }
        ...
        if (!gui.getMinecraft().options.hideGui && gui.shouldDrawSurvivalElements() && AsteorBar.Config.OVERWRITE_VANILLA_ARMOR_BAR.get()) {
            ...
            switch (Overlays.style) {
                ...
                case Overlays.STYLE_TOP_RIGHT -> {
                    ...
                    draw(poseStack, left, top, left + BOUND_FULL_WIDTH_SHORT, top + 5, armor, armorToughness, true);
                    Overlays.top += 6;
                }
