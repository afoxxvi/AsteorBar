...
public class FoodLevelOverlay extends BaseOverlay {
    ...
    private void draw(PoseStack poseStack, int left, int top, int right, int bottom, boolean highlight, int foodType, int foodLevel, float saturation, float exhaustion, boolean flip) {
        GuiHelper.drawTexturedRect(poseStack, left, top, right, bottom, 9, Y_FOOD_EMPTY_BAR, BOUND_FULL_WIDTH_LONG, 9);
        if (highlight) {
            GuiHelper.drawTexturedRect(poseStack, left, top, right, bottom, 9, Y_FOOD_BLINK_BOUND, BOUND_FULL_WIDTH_LONG, 9);
        }
        final int interWidth = right - left - 2;
        int foodWidth = (int) (interWidth * foodLevel / 20.0F);
        drawTextureFillFlip(poseStack, left + 1, top, right - 1, foodWidth, 9, 10, foodType, flip);
        if (AsteorBar.Config.DISPLAY_SATURATION.get()) {
            int saturationWidth = (int) ((right - left) * (saturation / 20.0F));
            drawTextureBoundFlip(poseStack, left, top, right, bottom, saturationWidth, 9, 9, Y_SATURATION_BOUND, BOUND_FULL_WIDTH_LONG, flip);
        }
        if (AsteorBar.Config.DISPLAY_EXHAUSTION.get()) {
            int exhaustionWidth = (int) (interWidth * (Math.min(4.0F, exhaustion) / 4.0F));
            drawTextureFillFlip(poseStack, left + 1, top, right - 1, exhaustionWidth, 9, 10, Y_FOOD_EXHAUSTION_FILL, FILL_FULL_WIDTH_LONG, flip);
        }
    }
