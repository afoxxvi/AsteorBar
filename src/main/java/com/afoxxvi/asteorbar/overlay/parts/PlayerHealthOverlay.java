...
public class PlayerHealthOverlay extends BaseOverlay {
    ...
    private void draw(GuiGraphics guiGraphics, int left, int top, int right, int bottom, boolean highlight, int healthType, float health, float absorb, float maxHealth, float flashAlpha, int regenerationOffset, String hp, boolean flip) {
        //draw bound
        GuiHelper.drawTexturedRect(guiGraphics, left, top, right, bottom, 9, Y_HEALTH_EMPTY_BAR, 182, 5);
        if (highlight) {
            GuiHelper.drawTexturedRect(guiGraphics, left, top, right, bottom, 9, Y_HEALTH_BLINK_BOUND, 182, 5);
        } else if (flashAlpha > 0) {
            RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, flashAlpha);
            RenderSystem.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA);
            GuiHelper.drawTexturedRect(guiGraphics, left, top, right, bottom, 9, Y_HEALTH_LOW_BOUND, 182, 5);
        ...
        drawTextureFillFlip(guiGraphics, left + 1, top, right - 1, healthLength, 5, 10, healthType, flip);
        //draw absorption
        if (absorbLength > 0) {
            if (flip) {
                drawTextureFillFlip(guiGraphics, left + 1, top, right - 1 - healthLength, absorbLength, 5, 10, Y_HEALTH_ABSORPTION_FILL, true);
            } else {
                drawTextureFillFlip(guiGraphics, left + 1 + healthLength, top, right - 1, absorbLength, 5, 10, Y_HEALTH_ABSORPTION_FILL, false);
            }
        }
        //draw regeneration
        if (regenerationOffset >= 0) {
            ...
            if (textureRight > 0) {
                GuiHelper.drawTexturedRect(guiGraphics, left + 1, top, 10 + 180 + textureLeft, Y_REGENERATION_FILL, -textureLeft, 5);
                GuiHelper.drawTexturedRect(guiGraphics, left + 1 - textureLeft, top, 10, Y_REGENERATION_FILL, textureRight, 5);
            } else {
                GuiHelper.drawTexturedRect(guiGraphics, left + 1, top, 10 + 180 + textureLeft, Y_REGENERATION_FILL, right - left - 2, 5);
            }
        }
        Overlays.addStringRender((left + right) / 2, top - 2, 0xFFFFFF, hp, Overlays.ALIGN_CENTER, true);
        //GuiHelper.drawCenteredString(guiGraphics, hp, (left + right) / 2, top - 3, 0xFFFFFF);
    }
        ...
        if (!gui.getMinecraft().options.hideGui && gui.shouldDrawSurvivalElements()) {
            ...
            switch (Overlays.style) {
                ...
                case Overlays.STYLE_ABOVE_HOT_BAR_LONG -> {
                    ...
                    gui.leftHeight += 12;
                    draw(guiGraphics, left, top, left + BOUND_FULL_WIDTH_LONG, top + 5, highlight, healthType, health, absorb, maxHealth, flashAlpha, regenerationOffset, hp, false);
                }
                case Overlays.STYLE_ABOVE_HOT_BAR_SHORT -> {
                    int top = screenHeight - gui.leftHeight + 4;
                    int left = screenWidth / 2 - 91;
                    gui.leftHeight += 6;
                    draw(guiGraphics, left, top, left + BOUND_FULL_WIDTH_SHORT, top + 5, highlight, healthType, health, absorb, maxHealth, flashAlpha, regenerationOffset, hp, false);
                }
                case Overlays.STYLE_TOP_LEFT -> {
                    int top = Overlays.top;
                    int left = Overlays.left;
                    draw(guiGraphics, left, top, left + Overlays.length, top + 5, highlight, healthType, health, absorb, maxHealth, flashAlpha, regenerationOffset, hp, false);
                    Overlays.top += 6;
                }
                case Overlays.STYLE_TOP_RIGHT -> {
                    int top = Overlays.top;
                    int left = screenWidth - Overlays.length - Overlays.left;
                    draw(guiGraphics, left, top, left + Overlays.length, top + 5, highlight, healthType, health, absorb, maxHealth, flashAlpha, regenerationOffset, hp, true);
                    Overlays.top += 6;
                }
